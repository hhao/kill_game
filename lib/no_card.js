...
$(function(){
	$("#cover_button").toggle(function(){
		$(".person_head_img").css({visibility:"hidden"});
	},
	function(){
		show_card++;
		$(".person_head_img").css({visibility:"visible"});
		alert('第'+show_card+'次查看底牌');
	});
	var showcard = function(){
		check_total++;
		console.log('第次'+check_total+'查验');
		if($(this).find('.person_head_img').attr('src')=='image/ic_begin_werewolf.png'){
			$(this).find('.person_head_img').css({visibility:"visible"});
			setTimeout("$('.person_head_img').css({visibility:'hidden'});", 3000);
		}else{
			alert('第'+check_total+'次查验：这是一个好人!');
		}
		$(".person_head").unbind('click', hidecard);
		$(".person_head").unbind('click', showcard);
	};
	var hidecard = function(){
		$(this).find('.person_head_img').css({visibility:"hidden"});
	};
	$("#check_button").click(function(){
		$(".person_head").toggle(hidecard, showcard);
	})
	$("#card").click(function(){						//产生角色分配界面
		...
		for(var i =0;i<person_number;i++){
			$("#container").append('<div id="person_card" data-role="page" data-url="" class="person_page"><div><img  class="front" src="image/empty_photo.png" alt="" /><img class="back" src="image/people.png" alt="" /><span></span></div><button>查看</button><a href="">下一个</a></div>');
		...
		if($("input[name='soldiers']").prop("checked")==true){			//士兵
			...
			$(".person_head_img").eq(soldiers_num).attr('src','image/ic_begin_guard.png');
			$(".person_page").eq(soldiers_num).find('.back').attr('src','image/ic_begin_guard.png');
		...
		$(".person_page button").toggle(function(){							//翻转图片，改变描述
			$(this).parents('.person_page').find('.front').hide();
		...
		function(){
			$(this).parents('.person_page').find('.front').show();
